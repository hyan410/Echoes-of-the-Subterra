...
export default class CanvasNodeFactory {
	...
	addGraphic = (type: GraphicType | string, layerName: string, options?: Record<string, any>): Graphic => {
		...
		switch(type){
			...
			case GraphicType.PARTICLE:
				...
				break;
			case GraphicType.LIGHT:
				instance = this.buildLight(options);
				break;
			default:
	...
	buildLight(options?: Record<string, any>): Light {
		this.checkIfPropExists("Light", options, "position", Vec2, "Vec2");
		this.checkIfPropExists("Light", options, "angle", "number", "number");
		this.checkIfPropExists("Light", options, "intensity", "number", "number");
		this.checkIfPropExists("Light", options, "distance", "number", "number");
		this.checkIfPropExists("Light", options, "opacity", "number", "number");
